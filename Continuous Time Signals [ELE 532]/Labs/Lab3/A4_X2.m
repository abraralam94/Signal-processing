function A3()% This function calculates the Dns from a user specified range given by user
    ...
    x = input(prompt);
    n = (-x:x);
    D_n = (1/2) * (sin((pi/2.0)*n)./((pi/2.0)*n)); %our X_2(t) Dn expression 
                                                   
    D_n(1,x+1) = 1/2.0;   % our D0 index, index = (1, given upper bound+1)
