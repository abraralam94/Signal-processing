% CH2MP4.m : Chapter 2, MATLAB Program 4 % see p.218 (Lathi) for further
% explanation
...
x = @(t) u(t-3) - u(t - 5);
h = @(t) 2*(u(t+5) - u(t+3));
dtau = 0.005; tau = -7:dtau:6;
ti = 0; tvec = -2.35:.1:4.75;
y = NaN*zeros(1,length(tvec)); % Pre-allocate memory
for t = tvec,
ti = ti+1; % Time index
xh = x(t-tau).*h(tau); lxh = length(xh);%x(t) is moving, while h(t) is fixed
